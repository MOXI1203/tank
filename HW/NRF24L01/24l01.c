...
{
	u8 status;	
   	NRF24L01_CSNL();                 //使能SPI传输
  	status =SPI1_ReadWriteByte(reg);//发送寄存器号 
  	SPI1_ReadWriteByte(value);      //写入寄存器的值
  	NRF24L01_CSNH();                 //禁止SPI传输	   
  	return(status);       			//返回状态值
}
//读取SPI寄存器值
//reg:要读的寄存器
...
{
	u8 sta;
 	SPI1_SetSpeed(SPI_CR1_BR_1);//spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   
	NRF24L01_CEL();
  	NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF  32个字节
 	NRF24L01_CEH();//启动发送	   
	xSemaphoreTake(BinaryNRTXDS_Handle,500);//等待发送完成
